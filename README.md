...
**Solution:** You're using the old script. Use `train_and_save_model.py` instead.

### Issue: Models not found
**Solution:** Run `python train_and_save_model.py` first to train and save models.
...
**Solution:** The script will automatically download it, but if it fails:
```python
import nltk
...
Edit `templates/index.html`, find the `examples` array:
```javascript
const examples = [
    "Your custom spam example here",
    "Your custom ham example here"
];
```
